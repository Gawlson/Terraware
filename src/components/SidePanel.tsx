    ...
    return (
        <aside className="w-80 flex flex-col h-full bg-[#F2E8CF] relative">
                ...
                {mode === 'fire' ? (
                    ...
                    <div className="flex flex-col items-center w-full">
                        <div className="flex flex-col gap-8 w-full max-w-[280px]">
                            ...
                            >
                                <h3 className="text-sm font-bold uppercase tracking-wider px-1" style={{ color: "#386641" }}>
                                    Identified Species in {point?.label}
                                </h3>
                                {speciesdata?.length > 0 ? speciesdata.map((s, index) => (
                                    ...
                                    <div className="flex items-center gap-2 px-1 py-3" style={{ color: "#6A994E" }}>
                                        <div className="w-4 h-4 rounded-full border-2 border-current border-t-transparent animate-spin" />
                                        <span className="text-sm">Analyzing data...</span>
                                    </div>
                                ) : (
                                    <p className="text-sm px-1 py-3" style={{ color: "#6A994E" }}>
                                        Click a state to see endangered species.
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        {point?.data && !point.loading && (
                            <div className="flex flex-col gap-3">
                                <DataCard
                                    ...
                                    value={
                                        <>
                                            {point.data.airqualitydata?.aqi ?? "—"}{" — "}
                                            <span style={{ color: aqiColor(point.data.airqualitydata?.aqi) }}>
                                                {aqiLabel(point.data.airqualitydata?.aqi) ?? ""}
                                            </span>
                                        </>
                                    }
                                    sub={
                                        point.data.airqualitydata?.dateObserved
                                ...
                                />
                                <DataCard
                                    label="Average Drought Level"
                                    value={
                                        <>
                                            {typeof droughtdata?.avgDroughtLevel === "number"
                                                ? droughtdata.avgDroughtLevel.toFixed(2)
                                                : "—"}
                                            {" — "}
                                            <span style={{ color: droughtdata?.severity?.color ?? "#6A994E" }}>
                                                {droughtdata?.severity?.label ?? "Loading…"}
                                            </span>
                                        </>
                                    }
            ...
            <div
                ...
                style={{ borderColor: "rgba(167, 201, 87, 0.5)", background: "#F2E8CF" }}
            >
                {mode !== "fire" && (
                    <button
                        onClick={handleOpenModal}
                        className="w-full flex items-center justify-center gap-2 rounded-none py-3 px-4 font-semibold text-sm border-2 transition-colors duration-200 bg-[#EDF5E0] text-[#386641] border-[#A7C957] hover:bg-[#386641] hover:text-[#EDF5E0] active:opacity-90"
                    >
                        <span className="text-base">✨</span>
                        <span>What you can do to help</span>
                    </button>
                )}
            </div>
